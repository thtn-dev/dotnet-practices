...
public class UserInfoController(UserManager<ApplicationUser> userManager) : Controller
{
    ...
    private async Task<Dictionary<string, object>> GetUserInfoClaims()
    {
        ...
        if (User.HasScope(OpenIddictConstants.Scopes.Profile))
        {
            ...
            claims[OpenIddictConstants.Claims.FamilyName] = user.UserName;
            claims[OpenIddictConstants.Claims.GivenName] = user.UserName;
        }
